...
def _request_path(environ):
    ...
    request bytes decoded as latin-1, so a UTF-8 path (e.g. a non-ASCII
    series folder name) arrives mangled and matches nothing on disk.
    Re-encoding to bytes and decoding as UTF-8 restores the real name. A
    pure ASCII path round trips unchanged, and anything that is not valid
    UTF-8 is left exactly as it arrived, so no path that works today
    changes behavior."""
