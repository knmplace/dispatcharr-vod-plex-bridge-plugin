...
- **Series/episode batch activation still needs care** — activating a large batch of episodes at once (e.g. "select all" on a full series) means Plex will HEAD/Range-probe every new STRM file on its next library scan, each of which opens a real provider connection. A per-episode capacity gate, STRM batching, and (as of v2.1.0) a global periodic max-concurrent sweep reduce this but don't eliminate the initial probe burst — activate a season or a handful of episodes at a time for providers with low concurrent-connection limits, not an entire multi-season series in one shot.
...
### v2.1.0 (2026-08-02)
- **New: Reactivate for episodes** — repeat-clicking "Activate" on an already-activated episode was previously a silent no-op (no STRM/NFO regen, no Plex scan, no error). Reactivate is a genuinely distinct action, mirroring the existing movies Reactivate: re-picks a stream with free provider capacity, regenerates STRM/NFO in place, and backfills `tvshow.nfo` if it's missing — deliberately does NOT touch Plex (no delete/scan; relies on Plex's own next scan to pick up the on-disk change). Available as a bulk "Reactivate Selected" button in the Series drill-down header and as a per-card ↻ icon on already-activated episodes.
- **Fixed: max concurrent connections per title not being enforced past the first redirect** — enforcement previously only ran when the plugin itself resolved a redirect, but rclone's VFS mount can go on hitting Dispatcharr directly with an already-issued redirect URL for minutes afterward, opening new real provider connections the plugin never saw. A new periodic sweep now runs every ~10 seconds independent of redirect resolution, scanning all live VOD sessions and pruning any title over its configured limit, so `Max Concurrent Connections Per Title` (Streaming settings) is now enforced continuously, not just at the moment of first playback.
- **Provider connection-limit guidance clarified** — the `Max Concurrent Connections Per Title` setting's help text now explicitly calls out that most IPTV providers cap total simultaneous streams per account (commonly 1-3), and every activated title sharing that account competes for the same pool — setting this too high, or leaving it at 0 (disabled), risks tripping your provider's own connection limit and getting sessions dropped/throttled at the provider level rather than gracefully by the plugin.

### v2.0.0 (2026-08-02)
- **Series/Episode activation** — full Series → Season → Episode browse and selective activation, mirroring the existing Movies flow: per-card checkboxes, Select All/Clear All, multi-select Activate/Deactivate into a configurable category (name + STRM folder + Plex library section, managed entirely from the dashboard's Series Settings tab). Generates the same STRM + NFO pattern as movies, triggers a Plex library scan, and supports Plex real-time delete on deactivation.
- Header stats now split "Movies Activated" / "Series Activated" counts.
- Trailer preview button added to series cards.
- **Known risk carried into this release**: batch-activating many episodes at once still means Plex will probe every new file on its next scan, each a real provider connection; a per-episode capacity gate and batched STRM writes reduce but don't eliminate this — see Known Limitations.
